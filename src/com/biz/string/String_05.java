...
public class String_05 {
	...
	public static void main(String[] args) {
		...
		if(bYes) 
			System.out.println("num에는 30이 담겨 있다");
		else
		...
		bYes = num == num1; //(해석) num에 담긴 값과 num1에 담긴 값이 같냐? 그 결과를 bYes에 담아라
		...
		if(bYes)
			System.out.println("두 문자열 일치"); // 두 문자열 일치
		else
		...
		// 아이디는 equalsIgnoreCase(대소문자 구분 x), 비밀번호는 equals(대소문자 구분)
		// 대소문자에 관계없이 단어나열이 일치한가?
		// equalsIgnoreCase썼을때 (K Orea != KOre A)는 다름(화이트스페이스 때문에 다름), (Korea == KOREA)는 같음
